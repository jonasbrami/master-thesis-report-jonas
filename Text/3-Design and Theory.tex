...
Removing the logarithm from the sink potential can be problematic for more complex environement because the field is no longer a solution of the Laplace equation \ref{eq:Laplace} and therefore we lose the guarantees regarding the absence of a local minimum in the velocity field.
...
Using potential based methods for the repulsive velocity field implementing obstacle avoidance is very efficient when there is a large amount of abstacles: When a new obstacle is detected/removed, we just need to add/remove its potential for the sum of current potentials.
